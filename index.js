...
document.getElementById('viewport').addEventListener('mousemove', (event) => {
    ...
    if (degree <= 180) {
        ...
        sector[0].style.backgroundImage = "linear-gradient(-"+0+"deg, #FFCCCB 50%, transparent 50%),linear-gradient("+(180-degree)+"deg, transparent 50%, #FFCCCB 50%)";
        sector[1].style.display = 'block';
        sector[1].style.backgroundImage = "linear-gradient(-"+0+"deg, white 50%, transparent 50%),linear-gradient("+180+"deg, transparent 50%, white 50%)";
        sector[2].style.display = 'none';
    } else {
        sector[2].style.display = 'block';
        sector[2].style.backgroundImage = "linear-gradient("+0+"deg, transparent 50%, #FFCCCB 50%),linear-gradient(-"+degree+"deg, #FFCCCB 50%, transparent 50%)";
    }
